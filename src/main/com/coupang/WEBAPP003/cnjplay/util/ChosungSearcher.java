package com.coupang.WEBAPP003.cnjplay.util;

public class ChosungSearcher {
	private static final char INITIAL_SOUND_BEGIN_UNICODE = 12593; // 초성 유니코드 시작
																	// 값
	private static final char INITIAL_SOUND_LAST_UNICODE = 12622; // 초성 유니코드 마지막
																	// 값
	...
	private static final char ENGLISH_ROWER_BEGIN_UNICODE = 65; // 영문(소문자) 유니코드
																// 시작 값
	private static final char ENGLISH_ROWER_LAST_UNICODE = 90; // 영문(소문자) 유니코드
																// 마지막 값
	private static final char ENGLISH_UPPER_BEGIN_UNICODE = 97; // 영문(대문자) 유니코드
																// 시작 값
	private static final char ENGLISH_UPPER_LAST_UNICODE = 122; // 영문(대문자) 유니코드
																// 마지막 값
	...
	private static boolean checkUnicode(char c) {
		if (((c >= NUMBER_BEGIN_UNICODE && c <= NUMBER_LAST_UNICODE)
				...
				|| (c >= HANGUL_BEGIN_UNICODE && c <= HANGUL_LAST_UNICODE) || (c >= INITIAL_SOUND_BEGIN_UNICODE && c <= INITIAL_SOUND_LAST_UNICODE))) {
			return true;
		} else {
			return false;
		}
		...
		for (int i = 0; i <= seof; i++) {
			...
			while (t < slen) {
				...
				if (isInitialSound(keyword.charAt(t))
					...
					// 각각의 초성이 같은지 비교
					if (getInitialSound(value.charAt(i + t)) == keyword
							.charAt(t)) {
						t++;
					} else {
						break;
